object Money {
    ...
    fun format(private var newValue: Any, private var oldValue: Any): Currency {
        private fun sanitizePrice(value: Any): String {
            var prevalue = value
            if (value is Float){
            ...
            var newVal =
                prevalue.replace(" ", "").replace(CURRENCY_SYMBOL, "").replace(HUNDRED_DELIMITER, "")
        ...
        private fun centsSanitizer(new: String, old: String): String {
        ...
        private fun hundredsSanitizer(new: String, old: String): String {
            ...
            val originalSplitted = oldSplitted[0].split("").filter { it.isNotEmpty() }.toMutableList()
