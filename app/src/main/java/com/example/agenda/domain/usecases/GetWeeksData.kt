...
) : Usecase<DayMonthYearObj,
        MutableMap<DayMonthYearObj, MonthYearObj>>, Subject<GetWeeksData> {
    override val observers: MutableList<Observer> = mutableListOf()
    override suspend fun execute(input: DayMonthYearObj):
            MutableMap<DayMonthYearObj, MonthYearObj> {
        val monthAndYearList = mutableMapOf<DayMonthYearObj, MonthYearObj>()
        val date = Date.getDate(DayMonthYearObj(1, input.month, input.year))
        val monthAndYear = GetWeekData(transactionRepository, eventRepository).execute(date)
        monthAndYearList[date] = monthAndYear
